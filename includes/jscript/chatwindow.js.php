...
var dbErrorFix = "<div style=\"border: 1px dashed rgb(204, 0, 0); font-family: Tahoma; background-color: rgb(251, 238, 235); width: 100%; padding: 10px; color: rgb(204, 0, 0);\"><strong>Critical Error</strong><br>Could not connect to the database</div>";
...
?>;
var currentSession = "<?= $_GET["currentSession"]; ?>";<?
require_once($dir."includes/chat/chatSession.php");
...
function getUpdates() {
	...
	if (!jQue) {
        ...
        jQuery("#receiver").load('includes/chat/chat.php', {count: count, session: currentSession, action: "view", wmessage: wMess}, function(responseText, textStatus, XMLHttpRequest) {
            ...
            if (jQuery("#receiver").text() != "" && jQuery("#receiver").text() != dbErrorFix) {               
...
<? } ?>
    ...
    <? } ?>
    <? if ($_GET["secretjoin"] != "true" && $utype == 2) { ?>
